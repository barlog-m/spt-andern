...
export class LootGenerator extends BotLootGenerator {
    ...
    public override generateLoot(
        ...
        const pocketLootCount = Number(
            this.weightedRandomHelper.getWeightedValue<number>(
        ...
        const stimItemCount = Number(
            this.weightedRandomHelper.getWeightedValue<number>(
                itemCounts.stims.weights,
            ),
        );
        const grenadeItemCount = Number(
            this.weightedRandomHelper.getWeightedValue<number>(
        ...
        const containersBotHasAvailable =
            this.getAvailableContainersBotCanStoreItemsIn(botInventory);
    ...
    protected generateBackpackLoot(
        sessionId: string,
        botLevel: number,
        botRole: string,
        botInventory: pmcInventory,
        ...
        const globalLootTableAvailable =
            backpackLootConfig.global.weights.length > 0;
        let selectedLootDataForBot: mapPmcBackpackLootData;

        if (mapLootTable && isPmc) {
            selectedLootDataForBot = this.getBotLootData(
                botLevel,
                mapLootTable,
            );
        }
        ...
        let backpackLootCount =
            config.pmcBackpackLoot && isPmc && selectedLootDataForBot
                ? this.randomUtil.getInt(
                      selectedLootDataForBot.min_items,
                      selectedLootDataForBot.max_items,
                  )
                : this.weightedRandomHelper.getWeightedValue<number>(
                      botJsonTemplate.generation.items.backpackLoot.weights,
                  );

        const containersBotHasAvailable =
            this.getAvailableContainersBotCanStoreItemsIn(botInventory);
        const canAddBackpackLoot =
            containersBotHasAvailable.includes(EquipmentSlots.BACKPACK) &&
            !config.disableBotBackpackLoot;
        const canUseCustomBackpackLoot =
            mapLootTable || globalLootTableAvailable;

        if (!canAddBackpackLoot || !canUseCustomBackpackLoot) {
            return;
        }

        const canAddWeaponsToBackpack =
            backpackLootConfig.include_weapons &&
            this.randomUtil.getChance100(
                this.pmcConfig.looseWeaponInBackpackChancePercent,
            );
        ...
        const shouldAddKeycard = this.randomUtil.getChance100(
            (selectedLootDataForBot?.keycard_chance ||
                backpackLootConfig.global.keycard_chance) * 100,
        );
        const shouldAddRareKey = this.randomUtil.getChance100(
            (selectedLootDataForBot?.rare_key_chance ||
                backpackLootConfig.global.rare_key_chance) * 100,
        );

        if (shouldAddKeycard) {
            const keycardTpl = this.randomUtil.getArrayValue(
                this.randomUtil.shuffle(
                    backpackLootConfig.loot_tables.keycards,
                ),
            );
            this.addLootFromPool(
                { [keycardTpl]: 1 },
        ...
        if (shouldAddRareKey) {
            const rareKeyTpl = this.randomUtil.getArrayValue(
                this.randomUtil.shuffle(
                    backpackLootConfig.loot_tables.rare_keys,
                ),
            );
            this.addLootFromPool(
                { [rareKeyTpl]: 1 },
        ...
        if (mapLootTable) {
            const combinedLootTable = combineMapItemListIntoArray(
                selectedLootDataForBot,
            );
            const itemTplsAdded: Record<string, number> = {};

            this.addLootFromList(
                this.getBackpackLootItemList(
                    combinedLootTable,
                    selectedLootDataForBot.weights,
                    backpackLootCount,
                    itemTplsAdded,
                ),
                [EquipmentSlots.BACKPACK],
        ...
        if (globalLootTableAvailable && backpackLootCount > 0) {
            ...
            this.addLootFromList(
                this.getBackpackLootItemList(
                    combinedLootTable,
                    backpackLootConfig.global.weights,
                    backpackLootCount,
                    itemTplsAdded,
                ),
                [EquipmentSlots.BACKPACK],
    ...
    protected getMapData(
        raidInfo: RaidInfo,
    ): mapPmcBackpackLootDataGroup | null {
        if (
            Object.prototype.hasOwnProperty.call(
                backpackLootConfig.custom_map_loot_tables,
                raidInfo.location,
            )
        ) {
    ...
    protected getBotLootData(
        botLevel: number,
        lootTableGroups: mapPmcBackpackLootDataGroup,
    ): mapPmcBackpackLootData {
        return lootTableGroups.find(
            (group) =>
                botLevel >= group.min_level && botLevel <= group.max_level,
        );
    }

    protected getBackpackLootItem(
        lootTableArray: string[][],
        weights: number[],
        botAddedBackpackItems: Record<string, number>,
    ): string {
        const selectedLootTable = this.weightedRandomHelper.weightedRandom(
            lootTableArray,
            weights,
        );

        let selectedItemTpl: string = this.randomUtil.getArrayValue(
            this.randomUtil.shuffle(selectedLootTable.item),
        );

        while (
            Object.prototype.hasOwnProperty.call(
                botAddedBackpackItems,
                selectedItemTpl,
            ) &&
            botAddedBackpackItems[selectedItemTpl] >=
                backpackLootConfig.duplicate_item_limit
        ) {
            selectedItemTpl = this.randomUtil.getArrayValue(
                this.randomUtil.shuffle(selectedLootTable.item),
            );
        }
    ...
    protected getBackpackLootItemList(
        lootTableArray: string[][],
        weights: number[],
        amountToAdd: number,
        botAddedBackpackItems: Record<string, number>,
    ): string[] {
        return Array.from({ length: amountToAdd }, () => "").map(() =>
            this.getBackpackLootItem(
                lootTableArray,
                weights,
                botAddedBackpackItems,
            ),
        ...
        if (itemListSize > 0) {
            ...
            for (let i = 0; i < itemListSize; i++) {
                ...
                if (this.botConfig.walletLoot.walletTplPool.includes(itemTpl)) {
                    const addCurrencyToWallet = this.randomUtil.getChance100(
                        this.botConfig.walletLoot.chancePercent,
                    );
                    if (addCurrencyToWallet) {
                        const itemsToAdd = this.createWalletLoot(newRootItemId);

                        const containerGrid =
                            this.inventoryHelper.getContainerSlotMap(itemTpl);

                        const canAddToContainer =
                            this.inventoryHelper.canPlaceItemsInContainer(
                ...
                this.addRequiredChildItemsToParent(
                    itemToAddTemplate,
